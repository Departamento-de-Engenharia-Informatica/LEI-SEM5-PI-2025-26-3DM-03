  ...
  private createWarehouseModule(
    ...
    for (let i = -2; i <= 2; i++) {
      ...
      const frame = new THREE.Mesh(
        new THREE.BoxGeometry(winW + 3, winH + 3, 1.4),
        frameMat,
      );
      frame.position.set(cx, 6 + concreteHeight + winH * 0.8, d / 2 + 1.3);
      frame.castShadow = true;
      group.add(frame);

      const glass = new THREE.Mesh(
        new THREE.BoxGeometry(winW, winH, 1),
        glassMat,
      );
      glass.position.set(cx, frame.position.y, d / 2 + 1.9);
      group.add(glass);

      // divisões internas
      const mullionH = new THREE.Mesh(
        new THREE.BoxGeometry(0.5, winH + 1, 1.05),
        frameMat,
      );
      mullionH.position.set(cx, frame.position.y, d / 2 + 2.0);
      group.add(mullionH);

      const mullionV = new THREE.Mesh(
        new THREE.BoxGeometry(winW + 1, 0.5, 1.05),
        frameMat,
      );
      mullionV.position.set(cx, frame.position.y, d / 2 + 2.0);
      group.add(mullionV);
    }
    ...
    const doorFrame = new THREE.Mesh(
      new THREE.BoxGeometry(doorWidth + 3, doorHeight + 3, 1.6),
      doorFrameMat,
    );
    doorFrame.position.set(
      0,
      6 + concreteHeight / 2 + doorHeight / 2,
      d / 2 + 1.6,
    );
    ...
    const doorPanel = new THREE.Mesh(
      new THREE.BoxGeometry(doorWidth, doorHeight, 1.2),
      doorPanelMat,
    );
    ...
    // retângulos horizontais no portão
    const stripeGeom = new THREE.BoxGeometry(doorWidth - 2, 1.1, 1.25);
    for (let y = -2; y <= 2; y++) {
      const stripe = new THREE.Mesh(stripeGeom, doorFrameMat);
      stripe.position.set(
        doorPanel.position.x,
        doorPanel.position.y + y * 4.5,
        d / 2 + 2.9,
      );
      group.add(stripe);
    }

    // pequena cobertura acima do portão
    const canopy = new THREE.Mesh(
      new THREE.BoxGeometry(doorWidth + 8, 2, 6),
      eaveMat,
    );
    canopy.position.set(
      doorFrame.position.x,
      doorFrame.position.y + doorHeight / 2 + 2,
      d / 2 + 4.5,
    );
    ...
    // degrau em frente ao portão
    const step = new THREE.Mesh(
      new THREE.BoxGeometry(doorWidth + 12, 2, 10),
      plinthMat,
    );
    step.position.set(0, 7, d / 2 + 10);
    step.receiveShadow = true;
    group.add(step);

    // defensas/bumpers de camião ao lado do portão
    ...
    const bumperGeom = new THREE.BoxGeometry(3, 8, 6);
    const bumpL = new THREE.Mesh(bumperGeom, bumperMat);
    bumpL.position.set(-doorWidth / 2 - 4, 9, d / 2 + 8);
    const bumpR = bumpL.clone();
    bumpR.position.x = doorWidth / 2 + 4;
    group.add(bumpL, bumpR);
