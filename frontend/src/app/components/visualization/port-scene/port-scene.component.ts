...
export class PortSceneComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly containerTexture: THREE.Texture = (() => {
    ...
    // create an empty texture object which will be populated when the image loads
    const tex = new THREE.Texture();
    // attempt primary texture, fall back to ridge texture if it fails
    ...
    const applySettings = (t: THREE.Texture) => {
      ...
      // prefer sRGB encoding for color textures when available
      // older Three.js uses .encoding, newer uses .colorSpace; handle both
    ...
    loader.load(
      primary,
      (loaded) => {
        // copy loaded image into our texture container so existing materials update
        const ld: any = loaded;
      ...
      () => {
        ...
        loader.load(
          ...
          (err) => {
            ...
            // if both fail, use a generated ridge texture
  ...
  private readonly containerMaterials: THREE.MeshStandardMaterial[] = this.containerPalette.map((hex) => {
    const mat = new THREE.MeshStandardMaterial({
      color: hex,
      metalness: 0.05,
      roughness: 0.22,
      map: this.containerTexture,
      bumpMap: this.containerTexture,
      bumpScale: 0.48,
      flatShading: true,
    });
    return mat;
  });
  // slightly smaller containers so there is a small gap to the road
  private readonly containerGeometry = new THREE.BoxGeometry(30, 14, 60);
